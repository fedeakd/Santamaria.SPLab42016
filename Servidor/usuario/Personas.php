...
class Persona{
	...
	public static function TraerUnUsuario($mail,$clave,$usuario){
		$objetoAccesoDato = AccesoDatos::dameUnObjetoAcceso(); 
		$consulta =$objetoAccesoDato->RetornarConsulta("SELECT * from usuarios where mail='$mail' && clave='$clave' && usuario='$usuario'");
		$consulta->execute();
		$per= $consulta->fetchObject('Persona');
		return $per;       
	...
	public static function VerificarMailYUsuario($mail,$usuario){
		$objetoAccesoDato = AccesoDatos::dameUnObjetoAcceso();  
		$consulta =$objetoAccesoDato->RetornarConsulta("SELECT * from usuarios where mail='$mail' || usuario='$usuario'");
		$consulta->execute();
		return $consulta->rowCount();            
